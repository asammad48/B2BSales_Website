  ...
  return (
    <div className="space-y-16">
      <header className="relative h-[340px] rounded-[3rem] overflow-hidden flex items-center px-12 bg-gradient-to-r from-primary to-accent">
        <div className="relative z-20 max-w-2xl space-y-6">
          <motion.div
            ...
            className="inline-flex items-center gap-2 px-4 py-2 bg-white/15 text-white rounded-full"
          >
            <Star className="w-4 h-4" />
            <span className="text-xs font-black uppercase tracking-widest">Top Picks</span>
          </motion.div>
          ...
          >
            Curated products selected for reliability, demand, and consistent performance.
          </motion.p>
        </div>
      </header>
      ...
      <section className="space-y-8">
        <div className="flex items-end justify-between border-b border-border pb-6">
          <div className="space-y-2">
            <h2 className="text-3xl font-black uppercase tracking-tight">Featured Inventory</h2>
            <p className="text-text-muted">Hand-selected high performance stock for your shop.</p>
          </div>
        </div>
        ...
        {!isLoading && (data?.items || []).length === 0 && (
          <div className="glass-card p-8 text-center text-text-muted">No featured products found for this shop.</div>
        )}
      ...
      <section className="p-12 bg-primary/5 border border-primary/10 rounded-[3rem] space-y-6">
        <h2 className="text-3xl font-black uppercase tracking-tight">Why Featured?</h2>
        <p className="text-text-muted leading-relaxed">
          Featured items are selected based on quality history, stock continuity, and market demand so your team can reorder with confidence.
        </p>
        <button className="flex items-center gap-2 text-primary font-black uppercase tracking-widest text-xs group">
          Selection criteria <ArrowRight className="w-4 h-4 transition-transform group-hover:translate-x-1" />
        </button>
      </section>
    </div>
  );
