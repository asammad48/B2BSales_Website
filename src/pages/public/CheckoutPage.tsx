...
export function CheckoutPage() {
  ...
  const onSubmit = async (event: FormEvent) => {
    ...
    try {
      ...
      setOrderRef(ref);
      setIsSuccess(true);
      showSuccess(response.message || t('checkout.success.submitted', { orderNumber: ref }));
      clearCart();
      setNotes('');
    } catch (error: any) {
  ...
  if (isSuccess) {
    return (
      <div className="max-w-lg mx-auto py-20 text-center space-y-6">
        ...
        <div>
          <h1 className="text-2xl font-black mb-2">{t('checkout.success.submitted', { orderNumber: orderRef })}</h1>
  ...
  if (items.length === 0) {
    return (
      <div className="max-w-lg mx-auto py-20 text-center space-y-6">
        ...
        <div>
          <h1 className="text-2xl font-black mb-2">{t('checkout.emptyCart')}</h1>
        </div>
        <Link to="/products" className="btn-primary inline-flex">
          {t('checkout.browseProducts')}
        </Link>
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8">
      <div>
        <h1 className="text-3xl font-black tracking-tight mb-1">{t('checkout.title')}</h1>
        <p className="text-text-muted text-sm">{t('checkout.subtitle')}</p>
      </div>
      ...
      <form onSubmit={onSubmit} className="grid grid-cols-1 lg:grid-cols-[1fr_360px] gap-6 items-start">
        <div className="space-y-5">
                ...
                <div key={item.product?.id} className="flex items-center gap-4 px-6 py-4">
                  ...
                  <div className="flex-grow min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-0.5">
                      {item.product?.sku && (
                        <span className="text-[10px] font-bold uppercase tracking-widest text-text-muted">{item.product.sku}</span>
                      )}
          ...
          <div className="rounded-2xl border border-border overflow-hidden">
            <div className="flex items-center gap-3 px-6 py-4 bg-primary/5 border-b border-border">
              ...
              <h2 className="text-xs font-black uppercase tracking-widest text-text">{t('checkout.form.pickupShopLabel')}</h2>
            </div>
          ...
          <div className="rounded-2xl border border-border overflow-hidden">
            <div className="flex items-center gap-3 px-6 py-4 bg-primary/5 border-b border-border">
              ...
              <h2 className="text-xs font-black uppercase tracking-widest text-text">{t('checkout.form.notesLabel')}</h2>
            </div>
            <div className="p-6 bg-surface">
              <textarea
                value={notes}
        ...
        <div className="rounded-2xl border border-border overflow-hidden shadow-sm lg:sticky lg:top-24">
          <div className="bg-primary px-6 py-5">
            <p className="text-xs font-black uppercase tracking-widest text-white/60 mb-1">{t('checkout.summary.total')}</p>
            <div className="flex items-baseline gap-2">
              <span className="text-sm font-bold text-white/60">
                {hasMixedCurrencies ? '' : totalCurrency}
              </span>
              <span className="text-4xl font-black text-white leading-none">
                {hasMixedCurrencies ? t('checkout.summary.mixed') : total.toFixed(2)}
              </span>
            </div>
            <p className="text-xs text-white/40 mt-2 font-medium">{totalItems} item{totalItems !== 1 ? 's' : ''}</p>
          </div>
